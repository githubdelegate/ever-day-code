...
- (void)playBackgroundMusic:(NSURL *)fileUrl
{
	[self playBackgroundMusic:fileUrl forcePlay:NO];
}

- (void)playBackgroundMusic:(NSURL *)fileUrl forcePlay:(BOOL)forcePlay
{		
	...
	{
		debug(@"Sry, ipod is playing... can't play file %@",fileUrl);
		return;
	}
	
	if(self.backgroundAudio && [self.currentBackgroundAudioFile isEqualToString:[fileUrl absoluteString]])
	{
...
//	NSURL *fileURL = [[NSURL alloc] initFileURLWithPath: soundFilePath];
	...
	if(error)
	{
		...
		debug(@"***** ERROR creando backgroundMusicPlayer file:%@", fileUrl);
		[player release];
		return;
	}	
	
	self.currentBackgroundAudioFile = fileUrl;
